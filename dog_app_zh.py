...
human_in_human = [face_detector(human) for human in human_files_short]
dog_in_human= [face_detector(dog) for dog in dog_files_short]

print ('The percentage of human face detected in shorted human files: {}%'.format(sum(human_in_human)*100/100))
print ('The percentage of human face detected in shorted dog files: {}%'.format(sum(dog_in_human)*100/100))
...
# __回答:__
# 
# 实际使用中对用户提供含有清晰面部特征的人脸图像是不合理的。可以在处理图片数据的时候进行图像增强等处理方式增强人脸图像的清晰度后再进行检测。
...
human_in_dog = [dog_detector(human) for human in human_files_short]
dog_in_dog= [dog_detector(dog) for dog in dog_files_short]

print ('The percentage of dog detected in shorted human files: {}%'.format(sum(human_in_dog)*100/100))
print ('The percentage of dog detected in shorted dog files: {}%'.format(sum(dog_in_dog)*100/100))
...
# __回答:__ 
# 
# 根据上图提示的步骤搭建了卷积网络，此架构能在该问题上取得很好的表现是由于：
# 1. 卷积神经网络在处理图像时可以高效利用数据，只需更少的训练样本就能学到具有泛化能力的数据表示；卷积神经网络可以有效学习复杂和抽象的视觉概念。
# 2. 但是光有卷积神经网络的话不利于学习特征的空间层次结构，需要让最后一个卷积层的特征包含输入的整体信息且元素不是很多。所以使用最大池化是为了：一是减少需要处理的特征图的元素个数，二是通过让连续卷积层的观察窗口覆盖原始输入的比例越来越大。
# 3. 用最大池化而不是平均池化，是因为观察不同特征的最大值而不是平均值能给出更多的信息，不会错过或淡化特征是否存在的信息。
...
# __回答:__ 
# 
# 1. 通过预训练模型作为检查点开始训练生成神经网络模型实现对新任务的支持，这种方法通常被称为迁移学习，它的好处是不用再重头开始设计与训练一个全新的网络，而是基于已经训练好的网络模型，在其基础上进行参数与知识迁移，只需要很少量的计算资源开销与训练时间就可以实现对新任务的支持。
# 2. Xception是Inception架构的扩展，它用深度可分离的卷积代替了标准的Inception模块,它的weight数量很少，而VGG网络架构weight数量相当大，很消耗磁盘空间，训练非常慢。
# 3. 由于新数据集很小，新数据与原始数据相似，使用Xception最后一层卷积层的输出被直接输入到模型。添加一个全局平均池化层以及一个全连接层，其中全连接层使用 softmax 激活函数，对每一个狗的种类都包含一个节点。
...
history = Xception_model.fit(train_Xception, train_targets, 
          ...
          epochs=17, batch_size=20, callbacks=[checkpointer], verbose=1)
...
def dog_predict(img_path):
    if dog_detector(img_path)== True:
        ...
        print('Error! You are not human or dog.')
...
# 
# 1. 输出的结果比我预想的一样
# 2. 想法：对训练集进行数据增强、对模型进行微调、调整模型的epoch，batch_size和激活函数。
...
## 自由地使用所需的代码单元数吧
dog_predict('images/American_water_spaniel_00648.jpg')


# In[36]:


dog_predict('images/128355b5e560c3898708db54656e13e6.jpg')


# In[37]:


dog_predict('images/c4a4db21d25062914ccf2ea231aee405.jpg')


# In[38]:


dog_predict('images/d439b6003af33a87d45d2296cf5c10385343b50d.jpg')


# In[39]:


dog_predict('images/timg.jpeg')


# In[40]:


dog_predict('images/d647823ed0b44d3b60f84702347fe94d.jpg')
